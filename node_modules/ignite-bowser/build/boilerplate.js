...
exports.install = exports.isAndroidInstalled = void 0;
const ramda_1 = require("ramda");
const react_native_version_1 = require("./lib/react-native-version");
 ...
 */
exports.isAndroidInstalled = (toolbox) => {
 ...
 * Let's install.
 */
exports.install = async (toolbox) => {
    const { filesystem, parameters, ignite, reactNative, print, system, template, prompt, patching, strings, meta, } = toolbox;
    const { colors } = print;
    const { red, yellow, bold, gray, cyan } = colors;
    const isWindows = process.platform === "win32";
    const isMac = process.platform === "darwin";
    const reactNativeVersion = react_native_version_1.getReactNativeVersion(toolbox);
    if (parameters.options["dry-run"])
        return;
    const perfStart = new Date().getTime();
    // prints info in gray, indenting 2 spaces
    const printInfo = info => print.info(gray("  " +
        info
            .split("\n")
            .map(s => s.trim())
            .join("\n  ")));
    ...
    if (askAboutExpo) {
        useExpo = await prompt.confirm(`Would you like to use Expo on this project?\n${gray(`
...
And here: https://guides.cocoapods.org/using/getting-started.html
    ...
    let rnInstall;
    if (useExpo) {
        rnInstall = await expo_1.expo.install({ name, toolbox });
    }
    else {
    ...
    const androidInfo = exports.isAndroidInstalled(toolbox)
        ? ""
